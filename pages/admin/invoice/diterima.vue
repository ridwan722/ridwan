<template>
    ...
    <v-col>
      <v-breadcrumbs>
        <v-breadcrumbs-item active>
          <span class="font-weight-medium text-h5"> Invoice Diterima </span>
        </v-breadcrumbs-item>
      </v-breadcrumbs>
    </v-col>
  </v-row>
  ...
  <v-card flat rounded="lg" class="border">
    ...
    >
      ...
      <template v-slot:item.tanda_terima_at="{ item }">
        <span>{{ rubahtanggalunix(item.tanda_terima_at) }}</span>
      </template>
...
const dataDiterima = computed(() =>
  invoicestore.dataInvoice.filter((item) => item.status === "Diterima"),
);

onMounted(async () => {
  await invoicestore.tarikdataInvoicebystatus("Diterima");
...
const filterStatus = ref("Diterima");

const filteredInvoices = computed(() => {
  const allData = invoicestore.getDataInvoice;
  if (filterStatus.value === "Diterima") return allData;
