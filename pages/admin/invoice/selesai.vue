...
const dataSelesai = computed(() =>
  invoicestore.dataInvoice.filter((item) => item.status === "Selesai"),
);

onMounted(async () => {
  await invoicestore.tarikdataInvoicebystatus("Selesai");
...
const filterStatus = ref("Selesai");

const filteredInvoices = computed(() => {
  const allData = invoicestore.getDataInvoice;
  if (filterStatus.value === "Selesai") return allData;
