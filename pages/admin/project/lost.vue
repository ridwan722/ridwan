    ...
    <v-col cols="10">
      <v-breadcrumbs>
        <v-breadcrumbs-item active>
          <span class="font-weight-medium text-h5"> Project Lost</span>
        </v-breadcrumbs-item>
      </v-breadcrumbs>
    </v-col>
  </v-row>
    ...
    <v-card-title class="pa-3">
      <v-row align="center">
        ...
        <v-col cols="12" sm="2" class="text-caption text-grey-darken-1">
          Total:
          {{ penawaranStore.getDataPenawaran.length }} penawaran
        </v-col>
      </v-row>
    </v-card-title>
      ...
      <template v-slot:bottom>
        ...
        <v-row class="pa-3" align="center">
          <v-col cols="12" md="7" class="d-flex align-center flex-wrap">
            <div class="text-caption text-grey-darken-1 me-4">
              Menampilkan
              <strong>{{ (data.page - 1) * data.itemsPerPage + 1 }}</strong> –
              <strong>{{
                Math.min(
              ...
              dari
              <strong>{{ penawaranStore.getDataPenawaran.length }}</strong> data
            </div>
            ...
            >
              ...
              <div class="d-flex align-center">
                ...
                <span class="text-xxs text-grey-darken-1">Kritis (<5 hr)</span>
              </div>
            </div>
          </v-col>

          <v-col cols="12" md="5" class="d-flex justify-md-end justify-center">
            <v-pagination
              v-model="data.page"
              :length="
                Math.ceil(
                  penawaranStore.getDataPenawaran.length / data.itemsPerPage
                )
              "
              total-visible="5"
              density="comfortable"
...
const data = reactive({
  ...
  headPenawaran: [
    ...
    { title: "Total", value: "total", sortable: true, align: "end" as const },
    {
      title: "Status",
      value: "status",
      sortable: true,
