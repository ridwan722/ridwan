<template>
    ...
    <v-card class="rounded-xl overflow-hidden elevation-3 border-0">
      <v-card-item class="bg-grey-lighten-4 pa-3 text-center">
        <h4 class="font-weight-bold text-grey-darken-3">
          ...
          <span class="text-primary">Aresa Digital</span>
        </h4>
  ...
  <v-row align="center">
    <v-col cols="12">
      <v-breadcrumbs>
        <v-breadcrumbs-item>
          <span class="font-weight-medium text-h5">Invoice Aresa Digital</span>
        </v-breadcrumbs-item>
      </v-breadcrumbs>
    </v-col>
  </v-row>
          ...
          <template v-slot:item.aksi="{ item }">
            <div class="d-flex justify-center">
              <v-btn
                ...
                @click="router.push(`/admin/invoice-aresa-digital/${item.id}`)"
              >
                <v-icon icon="mdi-eye" />
                <v-tooltip activator="parent" location="top"
...
async function simpanInvoiceDialog() {
  ...
  if (data.invoiceAddEdit === "add") {
    ...
    newInvoice.value = emptyInvoice();
    await router.push(`/admin/invoice-aresa-digital/${result.id}`);
    return;
  }
