    ...
    <v-col cols="10">
      <v-breadcrumbs>
        <v-breadcrumbs-item active>
          <span class="font-weight-medium text-h5">Draft Penawaran </span>
        </v-breadcrumbs-item>
      </v-breadcrumbs>
    </v-col>
  </v-row>
  ...
  <v-card flat rounded="lg" class="border">
    ...
    >
      ...
      <template v-slot:item.perihal="{ item }">
        <div style="width: 180px" class="font-weight-medium">
          {{ item.perihal }}
        </div>
      </template>
      ...
      <template v-slot:item.total="{ item }">
        <div style="width: 105px">
          <span class="font-weight-medium">Rp {{ rupiah(item.total) }}</span>
        </div>
      </template>
