    ...
    <v-col cols="10">
      <v-breadcrumbs>
        <v-breadcrumbs-item active>
          <span class="font-weight-medium text-h5"> Pemberkasan </span>
        </v-breadcrumbs-item>
      </v-breadcrumbs>
    </v-col>
  </v-row>
  ...
  <v-card flat rounded="lg" class="border">
    ...
    >
      ...
      <template v-slot:item.status="{ item }">
        <v-chip
         :color="
          ...
          rounded="lg"
        >
         <v-icon class="mr-2">mdi-folder-open-outline</v-icon> {{ item.status }}
        </v-chip>
      </template>
      ...
      <template v-slot:item.perihal="{ item }">
         ...
         <div style="min-width: 220px;"> {{ item.perihal }}</div>
            ...
            >
              ...
              >
                ...
                <span class="text-caption font-weight-medium">
                  ...
                  <strong>{{ totalPemberkasan }}</strong>
...
onMounted(async () => {
  await penawaranStore.tarikdatapenawaranbystatus("Pemberkasan");
...
const data = reactive({
  ...
  headPenawaran: [
    ...
    // { title: "Object", value: "kategori_object", sortable: true },
...
const totalPemberkasan = computed(() => {
  return penawaranStore.getDataPenawaran.filter(
    (item: any) => item.status === 'Pemberkasan'
  ).length;
});
