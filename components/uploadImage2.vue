...
onMounted(() => {
  useuploadStore.setReset2();
});

async function uploadPicture() {
  await useuploadStore.simpanFileAction2(props.typefolder);
}
...
function onfilepicked(event: any) {
  ...
  useuploadStore.setnameFile2(filename);
  useuploadStore.setfile2(file);
  useuploadStore.setTypeFile2(file.type);
  uploadPicture();
}

const progress = computed(() => useuploadStore.getprogressUpload2);
